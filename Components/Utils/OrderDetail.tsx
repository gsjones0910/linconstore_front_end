...
const OrderDetail = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem('order')) {
      const tOrder = JSON.parse(localStorage.getItem('order'));
      setOrder(tOrder)
    }
      ...
      <Box mx={5} mt={5}>
        <Stack direction={"row"} alignItems={"center"}>
          <Stack direction={"row"} alignItems={"center"} gap={2}>
            <ArrowBack
              onClick={() => { router.back(); localStorage.removeItem('order') }}
              className={"pointer"}
              sx={{ color: "#0ba659" }}
            />
          </Stack>
            ...
            <Box bgcolor={"wheat"} borderRadius={"10px"} p={1}>
              ...
              <Box display={"flex"} justifyContent={"space-between"}>
                ...
                <Typography fontSize={12}>
                  {currencySymbol}&nbsp;&nbsp;
                  {(
                    (order?.productId.shipping[0][order?.shipping.toLowerCase()]
                      .price *
                      sellerRate) /
                    countryRate
                  ).toFixed(2)}
